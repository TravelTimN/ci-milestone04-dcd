...
def view_desserts():
        # sort: alphabetically
        sort_recipe_name = recipes.find().sort([("recipe_name", 1)])
        # sort: number of views
        #sort_views = recipes.find().sort([("views", -1)])

        # total number of recipes
        total_recipes = recipes.count()
        return render_template("view_desserts.html", recipes=sort_recipe_name, total_recipes=total_recipes)
...
def view_dessert(recipe_id, slugUrl):
        ...
        return render_template("view_dessert.html", recipe=recipe, full_ingredient=full_ingredient, units=units)
...
def add_dessert():
        ...
        for dessert in desserts.find().sort([("desserts", 1)]):
        ...
        for measurement in measurements.find().sort([("measurements", 1)]):
                measurement_name = measurement.get("measurement_unit")
                for item in measurement_name:
                        measurement_list.append(item)
        #measurement_list = sorted(measurement_list)
        
        return render_template("add_dessert.html", allergens=allergen_list, desserts=dessert_list, measurements=measurement_list)
...
def update_dessert(recipe_id, slugUrl):
        ...
        return render_template("update_dessert.html", recipe=recipe, allergens=allergen_list, desserts=dessert_list, measurements=measurement_list, ingredients=ingredients_list)
